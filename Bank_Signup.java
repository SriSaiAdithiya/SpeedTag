...
public class Bank_Signup extends JFrame {
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    String email = "";
                    Bank_Signup frame = new Bank_Signup(email);
     ...
     * Create the frame.
     */
    public Bank_Signup(String email) {
        this.email = email;
        ...
        btnNewButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                ...
                try {
                    ...
                    bankPst.setDouble(3, Double.parseDouble(textField_2.getText())); // Closing_Bal//bal
                    int Rows = bankPst.executeUpdate();
                    JOptionPane.showMessageDialog(btnNewButton, Rows+"Record added");
                    bankPst.close();

                    String addAccQuery = "UPDATE CUSTOMER SET Acc_No = ? WHERE Email = ?";
                    PreparedStatement addAccPst = con.prepareStatement(addAccQuery);
                    addAccPst.setString(1, textField_1.getText()); // Bank account number obtained from the bank insertion
                    addAccPst.setString(2, email); // Email obtained from the Customer class
